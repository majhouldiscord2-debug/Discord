...
export default function Stats() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      friends, incomingReqs, outgoingReqs, blocked, pending,
      dmChannels, groupDms, ownedServers, accountCreated, isBot, userAvatar, displayName, tag,
    };
      ...
      >
        ...
        <div className="ml-auto flex items-center gap-2">
          <span className="text-[11px] text-[#4e5058] font-medium uppercase tracking-wider">
            {stats.isBot ? "Bot" : "User"}
          </span>
          <button
            onClick={handleRefresh}
            disabled={msgLoading || refreshing}
            title="Refresh message stats"
            className="p-1.5 rounded-[6px] text-[#4e5058] hover:text-[#dbdee1] hover:bg-white/8 transition-all disabled:opacity-40"
          >
            ...
            {user && (
              <>
                ...
                <InfoRow
                  ...
                  icon={stats.isBot ? <Bot className="w-3.5 h-3.5" /> : <Shield className="w-3.5 h-3.5" />}
                />
