...
router.get("/tool-settings/:toolId", async (req, res) => {
  ...
  if (isNaN(toolId)) return res.status(400).json({ error: "Invalid toolId" });

  try {
    const rows = await db.select().from(toolSettingsTable).where(eq(toolSettingsTable.toolId, toolId));
    if (rows.length === 0) return res.json({ toolId, ...DEFAULT_SETTINGS });
    const row = rows[0];
    return res.json({
      toolId: row.toolId,
      ...
      quickWave: row.quickWave,
      servers: row.servers,
      messages: row.messages,
      serverConfigs: row.serverConfigs ?? null,
    });
  } catch {
...
router.put("/tool-settings/:toolId", async (req, res) => {
  ...
  const { autoJoin, smartMention, dmMode, delay, safeMode, quickWave, servers, messages, serverConfigs } = req.body;
  ...
  try {
    await db
      ...
      .values({
        ...
        safeMode: safeMode ?? DEFAULT_SETTINGS.safeMode,
        quickWave: quickWave ?? DEFAULT_SETTINGS.quickWave,
